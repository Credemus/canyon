...
public class ProcessRepository implements IProcessRepository {
	...
	private static Map g_processSourceCache = Collections
	...
	public BPEProcess getProcess(long processEntityOid)
		...
		try {
			...
			PBPEProcess process = (PBPEProcess) session.load(PBPEProcess.class,
					new Long(processEntityOid));

			BPEProcess result = process.getProcess();
			return result;
	...
	public BPEProcess getProcess(String processId, Long packageRevisionOid)
		...
		try {
			...
			query.setString("processId", processId);
			query.setLong("packageRevisionOid", packageRevisionOid.longValue());
		...
		Serializable find() throws RepositoryException {
			...
			try {
				...
				if (it.hasNext()) {
					...
					if (g_processSourceCache.containsKey(new Long(process.getEntityOid()))) {
						return (Serializable) g_processSourceCache.get(new Long(process
								.getEntityOid()));
					}
					...
					g_processSourceCache.put(new Long(process.getEntityOid()), source);
	...
	public void createProcess(long packageRevisionOid, BPEProcess process,
		...
		try {
			...
			g_processSourceCache
					.put(new Long(pprocess.getEntityOid()), processSource);
		} catch (HibernateException e) {
			log.error("Exception", e);
			throw new RepositoryException(e);
		} finally {
	...
	public void updateProcess(BPEProcess process, Serializable processSource)
		...
		try {
			...
			PBPEProcess pprocess = (PBPEProcess) session.load(PBPEProcess.class,
					new Long(process.getProcessEntityOid()));
			...
			Long key = new Long(process.getProcessEntityOid());
...
//			EPACKAGE ppp where ppp.packageID=pp.packageId);
			...
			while (it.hasNext()) {
				...
				if (g_processSourceCache.containsKey(new Long(process.getEntityOid()))) {
					source = (Serializable) g_processSourceCache.get(new Long(process
							.getEntityOid()));
				} else {
