...
public class Base64
{
  private static  char  g_alphabet[]  = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=".toCharArray();
  private static  byte  g_codes[]     = new byte[256];

  private static  char  g_altAlphabet[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_.".toCharArray();
  private static  byte  g_altCodes[] = new byte[256];
  ...
  public static byte[] decode( char data[] )
  {
    ...
    for ( int ix = 0; ix < data.length; ix++ ) {
      int  value  = g_codes[data[ix] & 0xff];
      if ( value < 0 && data[ix] != '=' ) {
        tempLen--;
      }
    }
   ...
   * @return      Description of the Return Value
   */
  public static byte[] altDecode( char data[] )
  {
    int   tempLen  = data.length;
    for ( int ix = 0; ix < data.length; ix++ ) {
      int  value  = g_altCodes[data[ix] & 0xff];
      if ( value < 0 && data[ix] != '.' ) {
        tempLen--;
      }
    }
   ...
   * @return      Description of the Return Value
   */
  public static char[] altEncode( byte data[] )
  {
    char  out[]  = new char[( ( data.length + 2 ) / 3 ) * 4];
    int   i    = 0;
    for ( int index = 0; i < data.length; index += 4 ) {
